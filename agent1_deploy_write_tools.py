"""Deploy the two graph-WRITE tools as graph-authorized loops in mind_kernel_v0:

    graph_upsert  -> executor graph_upsert_ref  (structured idempotent MERGE)
    graph_cypher  -> executor graph_cypher_ref  (raw write Cypher)

Both capabilities declare `graphWrite: allowed_with_write_password`. The runtime
requires a matching `password` argument on every call (verified server-side with
a constant-time compare against .mcp-write-password / MIND_WRITE_PASSWORD).
`tools/list` derives them from the active bindings — no hidden dispatcher.
"""
...
UPSERT_SCHEMA = {
    ...
    "properties": {
        ...
        "relations": {"type": "array", "items": {"type": "object",
                      "properties": {"source": {"type": "string"}, "relation": {"type": "string"},
                                     "target": {"type": "string"}},
...
TOOLS = [
    {
        "tool": "graph_upsert", "executor": "graph_upsert_ref", "schema": UPSERT_SCHEMA,
        "loop": "space:l2:mcp:graph-upsert-v0", "obj": "objective:l2:mcp:graph-upsert",
        "code": "code:l2:mcp:graph-upsert:v0", "contract": "contract:l2:mcp:graph-upsert-tool:v0",
        "cap": "capability:l2:mcp:graph-upsert:v0", "binding": "binding:l2:mcp:graph-upsert:v0",
        "promise": "Écriture structurée idempotente (MERGE par identité) bornée et password-gated.",
    },
    {
        "tool": "graph_cypher", "executor": "graph_cypher_ref", "schema": CYPHER_SCHEMA,
        "loop": "space:l2:mcp:graph-cypher-v0", "obj": "objective:l2:mcp:graph-cypher",
        "code": "code:l2:mcp:graph-cypher:v0", "contract": "contract:l2:mcp:graph-cypher-tool:v0",
        "cap": "capability:l2:mcp:graph-cypher:v0", "binding": "binding:l2:mcp:graph-cypher:v0",
        "promise": "Cypher d'écriture brut (puissant/dangereux), password-gated.",
    },
]
...
def main() -> int:
    ...
    for t in TOOLS:
        ...
        q("""MERGE (o {id:$id}) SET o:RuntimeNode, o.node_type='narrative', o.subtype='objective',
          ...
          {"id": t["obj"], "name": f"Graph write via {t['tool']} stays bounded and password-gated", "t": ts})
        ...
        for s, rel, o in [
            (t["loop"], "HAS_OBJECTIVE", t["obj"]), (t["loop"], "DEFINED_BY_CODE", t["code"]),
            (t["binding"], "BINDS_SERVER", SERVER_ID), (t["binding"], "BINDS_TOOL", t["contract"]),
            (t["binding"], "BINDS_CAPABILITY", t["cap"]), (t["binding"], "BINDS_LOOP", t["loop"]),
            (SERVER_ID, "EXPOSES", t["contract"]), (t["cap"], "IMPLEMENTED_BY", t["code"]),
        ]:
    ...
    out = {"phase": "deploy-write-tools", "tools": proof, "generatedAt": ts}
    print(json.dumps(out, ensure_ascii=False, indent=2))
    return 0 if all(v["active"] for v in proof.values()) else 3
